"""
RandomForestModel — Pipeline(preprocessor → RandomForestRegressor).
Реализация полностью идентична XGBoostModel с точностью до имени.
"""
...
class RandomForestModel(BaseMLModel):
    def __init__(self, models_dir: Path, metrics: ModelMetrics, background: pd.DataFrame):
        super().__init__(name=MODEL_RANDOM_FOREST, metrics=metrics, background=background)
        model_path = models_dir / FILE_RANDOM_FOREST
